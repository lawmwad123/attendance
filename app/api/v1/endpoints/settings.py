...
from app.schemas.settings import (
    ...
    NotificationSettings, BiometricSettings, StaffAttendanceSettings, 
    VisitorManagementSettings, SettingsSummary
)
...
async def update_school_settings(
    ...
    print(f"Returning updated settings: {result}")
    return result
...
async def get_settings_summary(
    ...
    return SettingsSummary(
        ...
        visitor_management=VisitorManagementSettings(
            visitor_management_enabled=settings_dict.get("visitor_management_enabled", True),
            visitor_approval_workflow=settings_dict.get("visitor_approval_workflow", "host_approve"),
            visitor_auto_approve_parent_visits=settings_dict.get("visitor_auto_approve_parent_visits", True),
            visitor_require_id_verification=settings_dict.get("visitor_require_id_verification", True),
            visitor_notify_host_on_arrival=settings_dict.get("visitor_notify_host_on_arrival", True),
            visitor_notify_parent_on_visitor=settings_dict.get("visitor_notify_parent_on_visitor", True),
            visitor_notify_security_on_overstay=settings_dict.get("visitor_notify_security_on_overstay", True),
            visitor_print_badges=settings_dict.get("visitor_print_badges", True),
            visitor_badge_expiry_hours=settings_dict.get("visitor_badge_expiry_hours", 8),
            visitor_enable_blacklist=settings_dict.get("visitor_enable_blacklist", True),
            visitor_enable_emergency_evacuation=settings_dict.get("visitor_enable_emergency_evacuation", True),
            visitor_integrate_with_gate_pass=settings_dict.get("visitor_integrate_with_gate_pass", True),
            visitor_enable_qr_codes=settings_dict.get("visitor_enable_qr_codes", True),
            visitor_allow_pre_registration=settings_dict.get("visitor_allow_pre_registration", True),
            visitor_pre_registration_hours_ahead=settings_dict.get("visitor_pre_registration_hours_ahead", 24),
            visitor_auto_approve_pre_registered=settings_dict.get("visitor_auto_approve_pre_registered", False),
            visitor_visiting_hours_start=settings_dict.get("visitor_visiting_hours_start", "09:00"),
            visitor_visiting_hours_end=settings_dict.get("visitor_visiting_hours_end", "16:00"),
            visitor_max_duration_hours=settings_dict.get("visitor_max_duration_hours", 2),
            visitor_auto_checkout_after_hours=settings_dict.get("visitor_auto_checkout_after_hours", 4)
        ),
